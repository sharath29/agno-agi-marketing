...
class MarketingKnowledgeBase:
    ...
    def __init__(
        self, chroma_path: str = "./data/knowledge_db", embedding_model: str = "all-MiniLM-L6-v2"
    ):
        ...
        # Will initialize knowledge on first access
    ...
    async def get_best_practices(self, topic: str, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        return await self.search_knowledge(query=topic, document_type="best_practice", limit=limit)

    async def get_templates(self, category: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Get templates for a specific category."""
        return await self.search_knowledge(
            query="", category=category, document_type="template", limit=limit
        )

    async def add_campaign_learnings(
        self,
        campaign_type: str,
...
class RAGKnowledgeRetriever:
    ...
    async def get_context_for_query(
        self, query: str, context_type: str = "general", max_context_length: int = 2000
    ) -> str:
        """Get relevant knowledge context for a query."""
        try:
            # Search for relevant knowledge
            knowledge_results = await self.knowledge_base.search_knowledge(query=query, limit=3)
    ...
    async def get_template_suggestions(self, category: str) -> List[Dict[str, str]]:
        ...
        return [
            {"title": template["title"], "content": template["content"], "tags": template["tags"]}
            for template in templates
        ]

    async def enhance_prompt_with_knowledge(
        self, base_prompt: str, knowledge_query: str, max_knowledge_tokens: int = 1000
    ) -> str:
        """Enhance a prompt with relevant knowledge context."""
        knowledge_context = await self.get_context_for_query(
            query=knowledge_query, max_context_length=max_knowledge_tokens
        )
...
class AgnoKnowledgeIntegration:
    """Integration with Agno's native knowledge system."""

    def __init__(self, knowledge_base: MarketingKnowledgeBase):
        self._knowledge_base = knowledge_base
        self._rag_retriever = RAGKnowledgeRetriever(knowledge_base)
        # Create AgentKnowledge instance for Agno compatibility
        self._agno_knowledge = AgentKnowledge()

    @property
    def agno_knowledge(self) -> AgentKnowledge:
        """Get the Agno knowledge instance."""
        return self._agno_knowledge

    async def search(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Search knowledge base (implements Agno Knowledge interface)."""
        return await self._knowledge_base.search_knowledge(query, limit=limit)

    async def get_context(self, query: str) -> str:
        """Get knowledge context for a query."""
        return await self._rag_retriever.get_context_for_query(query)
