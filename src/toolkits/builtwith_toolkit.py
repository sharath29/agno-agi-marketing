...
class BuiltWithToolkit(Toolkit):
    ...
    def get_technology_trends(
        ...
        """
        try:
            result = self._make_request("trends1/api.json", {
                "TECH": technology,
                "MONTHS": str(months)
            })
    ...
    @tool
    def get_competitor_technologies(
        self,
        ...
        try:
            ...
            key_technologies = [
                tech["name"] for tech in ref_profile["technologies"]
                if tech.get("category") in ["Analytics", "CMS", "E-commerce", "Marketing"]
            ][:3]  # Focus on top 3 key technologies
            ...
            top_competitors = sorted(
                competitors.items(),
                key=lambda x: len(x[1]["shared_technologies"]),
                reverse=True
            )[:limit]
