...
class MultiLayerMemoryManager:
    ...
    def _init_providers(self):
        ...
        self.providers["vector"] = VectorMemoryProvider(
            self.settings.database.chroma_db_path, self.settings.memory.embedding_model
        )
    ...
    async def retrieve_similar_conversations(
        ...
        return await self.providers["vector"].retrieve(
            {"content": query_text, "memory_type": "conversation"}, limit
        )
    ...
    async def get_conversation_history(self, session_id: str, limit: int = 20) -> List[MemoryEntry]:
        """Get conversation history for a session."""
        return await self.providers["short_term"].retrieve(
            {"memory_type": "conversation", "session_id": session_id}, limit
        )

    async def semantic_search(
        self, query_text: str, memory_type: Optional[str] = None, limit: int = 10
    ) -> List[MemoryEntry]:
