...
class LLMSettings(BaseSettings):
    ...
    def validate_api_keys(cls, v):
        # Skip validation for placeholder values or None
        if not v or v.startswith(("your_", "sk-", "claude-")):
            return v
        # Only validate if it looks like a real key
        if len(v) > 10 and not v.startswith(("sk-", "claude-")):
...
class MemorySettings(BaseSettings):
    ...
    memory_provider: Literal["redis", "sqlite", "memory"] = Field("sqlite", env="MEMORY_PROVIDER")
    vector_store_provider: Literal["chroma", "pinecone", "faiss"] = Field(
        "chroma", env="VECTOR_STORE_PROVIDER"
    )
...
class SecuritySettings(BaseSettings):
    ...
    jwt_secret_key: str = Field(
        "agno-marketing-jwt-secret-key-change-in-production-environment", env="JWT_SECRET_KEY"
    )
...
def validate_required_apis() -> dict[str, bool]:
    ...
    validations = {
        ...
        "salesforce": bool(
            settings.marketing_apis.salesforce_username
            and settings.marketing_apis.salesforce_password
        ),
